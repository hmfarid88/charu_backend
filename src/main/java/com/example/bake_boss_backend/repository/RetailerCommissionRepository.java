...
public interface RetailerCommissionRepository extends JpaRepository<RetailerCommission, Long> {
  ...
  List<RetailerCommission> findRetailerCommissionByDate(String username, LocalDate startDate, LocalDate endDate);
   ...
   @Query("SELECT new com.example.bake_boss_backend.dto.RetailerDetailsDTO(rp.date, 'No', 0.0, 0.0, 0.0, SUM(rp.amount)) FROM RetailerCommission rp WHERE rp.username = :username AND  rp.retailerName = :retailerName AND rp.date BETWEEN :startDate AND :endDate GROUP BY rp.date")
  ...
  @Query("SELECT new com.example.bake_boss_backend.dto.RetailerDetailsDTO(rp.date, 'No', 0.0, 0.0, 0.0, SUM(rp.amount)) FROM RetailerCommission rp JOIN RetailerInfo ri ON rp.retailerName = ri.retailerName WHERE ri.salesPerson = :salesPerson AND rp.retailerName = :retailerName AND rp.date BETWEEN :startDate AND :endDate GROUP BY rp.date")
